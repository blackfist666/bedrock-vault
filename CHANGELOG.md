...
- A world built from a marketplace template shows that template's picture when it has none of its own — so a world copied down from a Realm is no longer a blank row in the Vault. Worlds with no template behind them still show no picture rather than borrowing one (#11)
