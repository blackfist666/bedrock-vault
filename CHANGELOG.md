...
- Copying a world into the vault twice no longer leaves two of it. Copying a Realm slot again updates the world it made last time — saying so when nothing has changed, and keeping the previous copy as a backup when it has — and importing a file the vault already holds adds nothing (#7)
